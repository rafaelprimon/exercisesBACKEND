...
GROUP BY JOB_ID ORDER BY `média_salarial` DESC;
...
ROUND(AVG(SALARY),2) AS `media_salario` FROM hr.employees;

/*  6. Escreva uma query que exiba a quantidade de pessoas que trabalham como pessoas programadoras ( IT_PROG ). */
SELECT JOB_ID, COUNT(*) AS `total_colaboradores` FROM hr.employees
...
COUNT(*) `funcionarios`
FROM hr.employees
GROUP BY DEPARTMENT_ID HAVING `funcionarios` > 10;
...
SELECT employee_id,
first_name,
year(hire_date) FROM hr.employees;

/*  14. Escreva uma query que exiba as seguintes informações de cada funcionário: id , primeiro nome e dia do mês no qual foi contratado (exiba somente o dia). */
SELECT employee_id,
first_name,
day(hire_date) FROM hr.employees;

/*  15. Escreva uma query que exiba as seguintes informações de cada funcionário: id , primeiro nome e mês no qual foi contratado (exiba somente o mês). */
SELECT employee_id,
first_name,
month(hire_date) FROM hr.employees;
